...
age.post%>%
  bind_rows()%>%
  group_by(ID)%>%
  summarise(median=median(age),lowerCI=quantile(age,0.025),upperCI=quantile(age,0.975))%>%
  separate(ID,into=c("PairID","D_or_R"),sep="_",remove=F)%>%
  mutate(PairID=factor(PairID,levels=Pair_metadata$Pair))%>%
  ggplot(aes(x=D_or_R,y=median,ymin=lowerCI,ymax=upperCI,col=D_or_R))+
  geom_point()+
  geom_errorbar(width=0.2)+
  facet_grid(~PairID,scales="free",space="free")+
  theme_classic()
...
tree_age_vs_age_plot<-age.post%>%
  ...
  separate(ID,into=c("PairID","D_or_R"),sep="_",remove=F)%>%
  mutate(PairID=factor(PairID,levels=Pair_metadata$Pair))%>%
  left_join(Pair_metadata,by=c("PairID"="Pair"))%>%
  ggplot(aes(x=Age,y=median,ymin=lowerCI,ymax=upperCI,col=factor(PairID)))+
  geom_abline(slope=1,intercept=0,linetype=2)+
  geom_point(alpha=0.5)+
  geom_errorbar(alpha=0.5,width=2)+
  scale_x_continuous(limits=c(10,110),breaks=seq(10,110,10))+
  scale_y_continuous(limits=c(10,110),breaks=seq(10,110,10))+
  scale_color_brewer(palette="Paired")+
  facet_grid(~D_or_R)+
  theme_classic()+
  my_theme+
  theme(legend.key.height = unit(3,"mm"))+
  labs(x="Actual age",y="Predicted age from phylogeny",col="Pair ID")
ggsave(filename=paste0(plots_dir,"tree_age_vs_age_plot.pdf"),plot = tree_age_vs_age_plot,width=6,height = 2.5)
