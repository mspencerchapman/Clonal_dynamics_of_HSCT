...
resim_output_directory = toString(args[2])
resim_output_trees_directory = toString(args[3])
posterior_sample_file = toString(args[4])
post_obs_index=as.integer(args[5])
resim_job_index=as.integer(args[6])
n_sims_per_job=as.integer(args[7])
n_jobs_accumulated_per_post_obs=as.integer(args[8])
pair_ID=as.integer(args[9])
model_name = toString(args[10])

cat( "0.0. resim_conditionals_transplant.R main: post_obs_index = ", post_obs_index, "\n", sep = "\t" )
cat( "0.0. resim_conditionals_transplant.R main: resim_job_index = ", resim_job_index, "\n", sep = "\t" )
# readLines("stdin",n=1)
...
#
# Set seed for RNG;
# This ensures that for this "posterior observation",
# and for each "posterior observation specific-job" (identified by the "resim_job_index")
# within the current "job array",
# the RNG is set to a unique seed.
#

# n_jobs_accumulated = n_jobs_accumulated + sim_job_index
n_jobs_accumulated_per_post_obs = n_jobs_accumulated_per_post_obs + resim_job_index

large_int = n_jobs_accumulated_per_post_obs + ( post_obs_index * 1e6 )

#
# For each run, generate a random integer:
#

set.seed( large_int )

# run <- round( runif( n=1, min = 1e6, max = 2e6 ), digits = 0 )
run <- round( runif( n=1, min = 1e8, max = 2e8 ), digits = 0 )
cat( "0.0. resim_conditionals_transplant.R main: run = ", run, "\n", sep = "\t" )
# readLines("stdin",n=1)
...
if( !file.exists( resim_output_directory ) )
{
	dir.create( resim_output_directory )
	
}

# Create the output directory:
if( !file.exists( resim_output_trees_directory ) )
{
	dir.create( resim_output_trees_directory )
...
make_sim_table <- function( n_sims, post_obs_index, stat_names_vect )
{
	
cat( "0.0. make_sim_table: n_sims = ", n_sims, "\n", sep = "\t" )
# cat( "0.0. make_sim_table: length( param_names_vect ) = ", length( param_names_vect ), "\n", sep = "\t" )
cat( "0.0. make_sim_table: length( stat_names_vect ) = ", length( stat_names_vect ), "\n", sep = "\t" )
# readLines("stdin",n=1)
	
	# sim_col_names_vect = c( "seed", param_names_vect, stat_names_vect )
	sim_col_names_vect = c( "seed", "post_obs_index", stat_names_vect )
	sim_table = as.data.frame( matrix( NA, nrow = n_sims, ncol = length( sim_col_names_vect ) ) )
	names( sim_table ) <- sim_col_names_vect
	
	sim_table[[ "post_obs_index" ]] = rep( post_obs_index, n_sims )
	
cat( "1.0. make_sim_table: names( sim_table ) = ", names( sim_table ), "\n", sep = "\t" )
cat( "1.0. make_sim_table: dim( sim_table ) = ", dim( sim_table ), "\n", sep = "\t" )
# readLines("stdin",n=1)
...
cat( "0.0. update_sim_table: row_index = ", row_index, "\n", sep = "\t" )
cat( "0.0. update_sim_table: length( selected_col_names_vect ) = ", length( selected_col_names_vect ), "\n", sep = "\t" )
cat( "0.0. update_sim_table: selected_col_names_vect = ", selected_col_names_vect, "\n", sep = "\t" )
cat( "0.0. update_sim_table: length( selected_info_vect ) = ", length( selected_info_vect ), "\n", sep = "\t" )
cat( "0.0. update_sim_table: selected_info_vect = ", selected_info_vect, "\n", sep = "\t" )
# readLines("stdin",n=1)
	
cat( "0.0. update_sim_table: names( sim_table ) = ", names( sim_table ), "\n", sep = "\t" )
cat( "0.0. update_sim_table: dim( sim_table ) = ", dim( sim_table ), "\n", sep = "\t" )
# readLines("stdin",n=1)
...
cat( "1.0. update_sim_table: names( sim_table ) = ", names( sim_table ), "\n", sep = "\t" )
cat( "1.0. update_sim_table: dim( sim_table ) = ", dim( sim_table ), "\n", sep = "\t" )
# readLines("stdin",n=1)
...
sim_table_list = Map( function(x){ make_sim_table( nsim, post_obs_index, x ) }, stat_names_vect_list )
...
my_working_directory = resim_output_directory
setwd(my_working_directory)

####################################################################################################
####################################################################################################
# 
# Read file "posterior_sample_file"
# into data frame "posterior_table" columns:
#

# cols_params = c("target_pop_size","midlife_pop_change","latelife_pop_change")
...
#
# Specify parameter values in (named) vector "sim_params_vect";
# These parameters have the same value for all simulations within the loop!
#

sim_params_vect[ param_names ] <- unlist( posterior_table[ post_obs_index, param_names ] )


cat( "1.0. sim_prior_transplant.R main: sim_params_vect = ", sim_params_vect, "\n", sep = "\t" )
cat( "1.0. sim_prior_transplant.R main: names( sim_params_vect ) = ", names( sim_params_vect ), "\n", sep = "\t" )
cat( "1.0. sim_prior_transplant.R main: length( sim_params_vect ) = ", length( sim_params_vect ), "\n", sep = "\t" )
# readLines("stdin",n=1)
...
# param_posterior<-read.delim("/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/ABC_models/ABC_Apr2022/posterior_sample.txt") # kjd
# HSC_pop_posteriors<-read.delim("/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/ABC_models/ABC_Apr2022/KX001_KX002_combined_Nt_posterior.txt",header = F) # kjd
...
resim_tree_Rds_outfile = paste( resim_output_trees_directory, "/", "resim_trees_", "job_index_", resim_job_index, "_post_obs_", post_obs_index, "_run_", run, ".Rds", sep = "" )

####################################################################################################

# output_dir_list = lapply( stat_set_name_list, function(x){ paste( output_directory, x, "/", sep="" ) } )
output_dir_list = lapply( stat_set_name_list, function(x){ paste( resim_output_directory, x, "/", sep="" ) } )

outfile_list = lapply( output_dir_list, function(x){ paste( x, "resim_", "job_index_", resim_job_index, "_post_obs_", post_obs_index, "_run_", run, ".txt", sep="" ) } )
...
fitness_threshold = 0.05
  ...
  if( model_name == "m1" )
  {
	...
	HSC_pop_size = sim_params_vect[ "HSC_pop_size" ] # kjd
	HSCT_bottleneck = sim_params_vect[ "HSCT_bottleneck" ] # kjd
	number_drivers_per_year = sim_params_vect[ "number_drivers_per_year" ] # kjd
	gamma_shape = sim_params_vect[ "gamma_shape" ] # kjd
	gamma_rate = sim_params_vect[ "gamma_rate" ] # kjd
	# fitnessGammaFn = sim_params_vect[ "fitnessGammaFn" ] # kjd
	dpcpd = sim_params_vect[ "dpcpd" ] # kjd
  ...
  }else if( model_name == "m2" )
  {
	...
	gamma_shape_engraftment = sim_params_vect[ "gamma_shape_engraftment" ] # kjd
	gamma_rate_engraftment = sim_params_vect[ "gamma_rate_engraftment" ] # kjd
  ...
  }else if( model_name == "m3" )
  {
	...
	exaggeration_ratio = sim_params_vect[ "exaggeration_ratio" ] # kjd
	exaggeration_prop = sim_params_vect[ "exaggeration_prop" ] # kjd
...
# readLines("stdin",n=1)
  ...
  # sim_seed_string <- paste( "seed_", run+sim, sep = "" ) # kjd 16/06/2023
  
  # sim_params_vect = unlist( sim_table[ sim, param_names ] )
  # names( sim_params_vect ) <- param_names
  
  # sim_traject = list( params_vect = sim_params_vect, stats_vect = sim_stats_vect, traject_table = sim_traject_table )
  # sim_tree_info = list( seed = run+sim, params_vect = sim_params_vect, tree = DR_tree )
  sim_tree_info = list( seed = run+sim, post_obs_index = post_obs_index, tree = DR_tree )
  ...
  saveRDS( sim_tree_list, file = resim_tree_Rds_outfile )
