...
addDifferentiationEvents_engraftment_selection=function (tree, cfg, newCellType, idx = NULL, nEvent = -1, currentCompartment = -1) 
  {
  ...
  if (is.null(idx)) {
    ...
    if (currentCompartment < 0) {
      idx = sample(which(tree$state[idxt] > 0), size = nEvent, 
                   replace = FALSE)
    }
    else {
