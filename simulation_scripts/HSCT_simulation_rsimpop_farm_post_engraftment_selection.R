...
my_working_directory<-ifelse(Sys.info()["sysname"] == "Darwin","~/R_work/Clonal_dynamics_of_HSCT/","/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/")
...
checkCompartmentCofficients=function(tree){
  inf=tree$cfg$info %>% left_join(tree$events,by=c("id"="driverid","val"="value"))
  ##  For each entry find the ancestral drivers and recalculate the compartment coefficient
  inf$chk_fitness=sapply(inf$node,function(node){
    pnodes=rsimpop:::get_parents(node,tree$edge)
    alldrivers=tree$events %>% filter(driverid>0) %>% inner_join(data.frame(node=pnodes),by="node") %>% left_join(tree$cfg$drivers,by=c("driverid"="driver"))
    if(dim(alldrivers)[1]==0){
      0
    }else{
      sum(alldrivers$fitness)
    }
  })
  inf %>% select(population,val,fitness,id,driver1,chk_fitness)
}

resetSelectionCoefficient=function(tree,driverid,newFitness){
  tree$cfg$drivers=tree$cfg$drivers %>% mutate(fitness=ifelse(driverid==driver,newFitness,fitness))
  inf=checkCompartmentCofficients(tree)
  print(inf)
  tree$cfg$info=inf %>% mutate(fitness=chk_fitness) %>% select(-chk_fitness)
  tree
}
...
tree_folder=ifelse(Sys.info()["sysname"] == "Darwin","~/R_work/Clonal_dynamics_of_HSCT/data/trees_no_dups/","/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/filtering_runs2/tree_files")
...
param_posterior_file<-ifelse(Sys.info()["sysname"] == "Darwin","~/R_work/Clonal_dynamics_of_HSCT/data/posterior_sample.txt","/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/ABC_models/ABC_Apr2022/posterior_sample.txt")
HSC_pop_posteriors_file<-ifelse(Sys.info()["sysname"] == "Darwin","~/R_work/Clonal_dynamics_of_HSCT/data/KX001_KX002_combined_Nt_posterior.txt","/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/ABC_models/ABC_Apr2022/KX001_KX002_combined_Nt_posterior.txt")
param_posterior<-read.delim(param_posterior_file)
...
for(sim in 1:nsim) {
  ...
  #choose proportion of clones to have exaggerated selective advantage in the post-engraftment period
  recipient_driver_ids<-post_HSCT$cfg$drivers%>%filter(val==1 & driver>0)%>%pull(driver)
  exaggerated_postengraftment_selection_clones<-sample(recipient_driver_ids,size = round(exaggeration_prop*length(recipient_driver_ids)),replace=F)
  
  #Cycle through the drivers that will have enhanced postengraftment selection
  for(driverID in exaggerated_postengraftment_selection_clones) {
    cat(driverID,sep="/n")
    curr_fitness=post_HSCT$cfg$drivers%>%filter(driver==driverID)%>%pull(fitness)
    post_HSCT=resetSelectionCoefficient(tree = post_HSCT,driverid = driverID,newFitness = curr_fitness*exaggeration_ratio)
  }
  ...
  #Cycle through the drivers that have enhanced postengraftment selection and reset the fitness
  for(driverID in exaggerated_postengraftment_selection_clones) {
    curr_fitness=post_HSCT$cfg$drivers%>%filter(driver==driverID)%>%pull(fitness)
    post_HSCT=resetSelectionCoefficient(tree = post_HSCT,driverid = driverID,newFitness = curr_fitness/exaggeration_ratio)
  }
  ...
  write.tree(DR_tree_m,file=paste0(output_dir,"/tree_files/tree_",PairID,"_",uid,".tree"))
