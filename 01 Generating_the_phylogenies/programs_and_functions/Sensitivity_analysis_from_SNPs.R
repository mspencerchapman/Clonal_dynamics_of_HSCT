...
option_list = list(
  ...
  make_option(c("-x", "--xtension"), action="store", default="_complete_final_retained_3.vcf",type='character', help="Name for output_file"),
  ...
  make_option(c("-d", "--depth"), action="store", default=500, type='numeric', help="Minimum depth for classification of mutation as germline"),
  make_option(c("-r", "--rho"), action="store", default=0.05, type='numeric', help="Maximum beta-binomial rho value for classification of mutation at germline"),
  make_option(c("-b", "--binom"), action="store", default=6, type='numeric', help="-log10 of exact binomial test for mutation to come from somatic distribution - must be less than this value")
)
...
germline_INDELs = COMB_mats$mat$mut_ref[COMB_mats$mat$Mut_type == "INDEL" & log10(filter_params$germline_pval) > -binom_cutoff & filter_params$bb_rhoval < rho_cutoff  & rowSums(COMB_mats$NR) > depth_cutoff]

print(paste("There are",length(germline_SNVs),"defined germline SNVs and",length(germline_INDELs),"defined germline INDELs. Each sample will be tested for the proportion of these mutations that were called by Caveman/ Pindel."))
...
get_proportion_of_germline_detected = function(file, reference_germline_muts, skip = "#CHROM") { #By default will skip vcf header. If no header, set skip to 0.
  ...
  print(paste(file," completed. Pick up rate of",pick_up_rate,"calculated."))
  return(pick_up_rate)
}
...
create_sensitivity_df=function(germline_SNV_set, #A vector of germline SNVs in the format "Chrom-Pos-Ref-Alt" 
                               ...
                               SNV_calls_file_ext="_complete_final_retained_3.vcf", #The extension for the SNV vcf files (the extension will be removed to get sample names)
  ...
  #Get sensitivity for germline SNVs for each sample
	...
	SNV_sensitivity <- sapply(pass_SNV_files, get_proportion_of_germline_detected, skip=ifelse(grepl("flags|bed_file",SNV_calls_file_ext),0,"#CHROM"),reference_germline_muts = germline_SNV_set) #iterate through these files to get the sensitivity
	...
	if(!is.null(germline_INDEL_set)) {
	  ...
	  both_samples=intersect(names(SNV_sensitivity),names(INDEL_sensitivity)) #Only include samples which have both SNV and INDEL sensitivity
	  results_df=data.frame(Sample = both_samples, SNV_sensitivity = SNV_sensitivity[both_samples], INDEL_sensitivity = INDEL_sensitivity[both_samples])
...
sensitivity_analysis=create_sensitivity_df(germline_SNV_set=germline_SNVs,
                                                 ...
                                                 SNV_calls_file_ext=SNV_calls_file_ext,
