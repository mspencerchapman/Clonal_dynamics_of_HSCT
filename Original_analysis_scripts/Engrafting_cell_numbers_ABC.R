...
ggsave(filename = paste0(plots_dir,"HSC_pop_posteriors_plot.pdf"),HSC_pop_posteriors_plot,width=2.5,height=2)
ggsave(filename = paste0(plots_dir,"HSCT_bottleneck_posteriors_plot.pdf"),HSCT_bottleneck_posteriors_plot,width=2.5,height=2)
...
ggsave(filename = paste0(plots_dir,"ECN_by_age_at_sampling.pdf"),ECN_by_age_at_sampling,width=2.5,height=2)
ggsave(filename = paste0(plots_dir,"ECN_by_age_at_HSCT.pdf"),ECN_by_age_at_HSCT,width=2.5,height=2)
ggsave(filename = paste0(plots_dir,"ECN_by_CD34.pdf"),ECN_by_CD34,width=2.5,height=2)
ggsave(filename = paste0(plots_dir,"ECN_by_MNC.pdf"),ECN_by_MNC,width=2.5,height=2)
...
ggsave(filename = paste0(plots_dir,"ppc_selection_sumstats_plot.pdf"),ppc_selection_sumstats_plot,width=6,height=2)
...
ppc_postHSCT_division_select_sumstats_plot<-as.data.frame(posterior_checks)%>%
  dplyr::select(PairID,sumstats_to_visualize)%>%
  dplyr::filter(PairID%in%IDs_to_visualize)%>%
  ggplot(aes(x=get(sumstats_to_visualize[1]),y=get(sumstats_to_visualize[2])))+
  #geom_density_2d()+
  #stat_density_2d(aes(fill = ..level..), geom = "polygon")+
  geom_jitter(alpha=0.05,size=1,height=1,width=1)+
...
ggsave(filename = paste0(plots_dir,"ppc_postHSCT_division_select_sumstats_plot.pdf"),ppc_postHSCT_division_select_sumstats_plot,width=6,height=2)
