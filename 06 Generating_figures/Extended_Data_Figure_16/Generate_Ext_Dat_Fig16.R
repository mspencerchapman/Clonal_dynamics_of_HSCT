...
driver_FC_by_Gene_plot<-driver_FC_df%>%
  ...
  labs(x="",y=bquote(~Log[2]~Fold~Change~(Recip/ Donor)),col="")+
  facet_grid(mut_cat~cell_type,scales = "free",space="free")+
  coord_flip()+
  theme_bw()+
  my_theme+
  theme(axis.title.y = element_blank(),axis.text.x = element_text(size=5),strip.text.x = element_text(size=7),strip.text.y = element_text(size=7,face="italic"),axis.text.y = element_text(size=6,face="italic"),axis.title = element_text(size=7))

ggsave(filename = paste0(plots_dir,"ExtDatFig16.pdf"),driver_FC_by_Gene_plot,device = "pdf",width=7,height = 9)
