...
#========================================#
cran_packages=c("abc","ggridges","phangorn","ggplot2","tidyr","dplyr","RColorBrewer","tibble","ape","dichromat","seqinr","stringr","readr","phytools","devtools","lme4","lmerTest")
...
root_dir<-ifelse(Sys.info()["sysname"] == "Darwin","~/R_work/Clonal_dynamics_of_HSCT","/lustre/scratch126/casm/team154pc/ms56/Zur_HSCT")
source(paste0(root_dir,"/data/HSCT_functions.R"))
plots_dir=paste0(root_dir,"/plots/")
...
## Perform lmer including age only ----
lmer.age_only <- lmer(tree_age ~ Age + (1|PairID), data = age.post.for.lme)
summary(lmer.age_only)

## Perform lmer including age and donor/ recipient status as a fixed effect----
...
## Plot the results of the ABC with the regression lines -----
lme.plot<-age.post.for.lme%>%
  mutate(D_or_R=substr(D_or_R,1,1))%>%
  ggplot(aes(x=Age,y=tree_age,col=factor(D_or_R)))+
  ...
  scale_color_manual(values=DorR_cols,labels=c("Donor","Recipient"))+
  scale_x_continuous(limits=c(20,90),breaks=seq(0,100,10))+
  scale_y_continuous(limits=c(0,110),breaks=seq(0,100,10))+
  guides(color = guide_legend(override.aes = list(alpha = 1,size=1)))+
  theme_classic()+
  my_theme+
  theme(legend.spacing = unit(0,"mm"),legend.margin=margin(0,0,0,0),legend.key.size = unit(3, "mm"),
        legend.box.margin=margin(-10,0,-10,-10))+
  labs(col="",x="Actual age (years)",y="Tree age posterior (years)")

ggsave(filename=paste0(plots_dir,"Fig3c.pdf"),plot = lme.plot,width=3,height = 2.5)

#========================================#
# A COUPLE OF ADDITIONAL PLOTS (not in manuscript) ####
#========================================#
