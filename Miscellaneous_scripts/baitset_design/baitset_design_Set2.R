...
if(file.exists(paste0(output_dir,"all.muts.RDS"))) {
  ...
  all.muts<-all.muts[c("Pair38","Pair40","Pair41")]
}
...
all.muts%>%
  ...
  write.table(file="baitset2_SNVs.bed",quote=F,sep="\t",row.names=F,col.names = F)

all.muts%>%
  dplyr::bind_rows()%>%
  filter(covered_final==1 & Mut_type=="INDEL")%>%
  dplyr::select(Chrom,Pos,Ref,Alt)%>%
  write.table(file="baitset2_INDELs.bed",quote=F,sep="\t",row.names=F,col.names = F)
