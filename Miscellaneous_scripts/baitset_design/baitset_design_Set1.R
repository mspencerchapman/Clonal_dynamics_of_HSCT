...
write_mut_coords_bedfile=function(details,all_mut_coords_path) {
  ...
  write_delim(mut_batch_coords, path = all_mut_coords_path, delim = "\t", col_names = FALSE)
}

read_in_bait_set_covered_regions=function(dir,stringency,file_suffix="_t2_1_Covered.bed") {
  files=list.files(dir,pattern = paste0(stringency,file_suffix),full.names = T)
...
output_dir="Baitset1/"
exp_nos=c(11,13,28,31,38,40,41)
...
if(file.exists(paste0(output_dir,"all.muts.RDS"))) {
  ...
  annotated_muts_paths=list.files(annotated_muts_folder,pattern="a_j_pval_post_mix_post_dup",full.names = T)
  ...
  all.muts<-all.muts[c("Pair11","Pair13","Pair28","Pair31")]
}

tree_paths=list.files(tree_folder,pattern="a_j_pval_post_mix_post_dup.tree",full.names = T)
...
if(!"bait_set"%in%colnames(all.muts[[1]])) {
  ...
  for(stringency in c("least","mod","most")) {
    cat_bed=read_in_bait_set_covered_regions(dir=paste0(output_dir,"/Covered_region_beds"),stringency=stringency,file_suffix="_\\d_Covered.bed")
...
if(!file.exists(paste0(output_dir,"All_probegroups_HSCT_Set2_by_target"))) {
  replication_info_df=read.delim(paste0(output_dir,"All_probegroups_HSCT_Set2_least_GC.txt"))
  ...
  write.table(replication_info_df,file=paste0(output_dir,"All_probegroups_HSCT_Set2_by_target"),sep = "\t",quote=F,row.names=F)
 } else {
 replication_info_df<-read.delim(paste0(output_dir,"All_probegroups_HSCT_Set2_by_target"),stringsAsFactors = F)
}

all.muts<-lapply(all.muts, function(details) {
  if("Total_rep"%in%colnames(details)) {
    return(details)
  } else {
    details<-left_join(details,replication_info_df%>%select(Chrom,Pos,ProbeIDs,Total_rep),by=c("Chrom","Pos"))
    return(details)
  }
})
...
samples_for_specificity_analysis=c("PD45813b_lo0024","PD45813b_lo0028","PD45813b_lo0111","PD45808b_lo0003","PD45808b_lo0008")
...
all.muts<-Map(details=all.muts,pair=names(all.muts),tree=all.trees,f=function(details,pair,tree) {
  ...
  private_muts = details$mut_ref[details$node %in% 1:length(tree$tip.label) & details$Mut_type == "SNV" & details$bait_set == "most" &details$Total_rep<=4] #Only private mutations that pass stringent masking, stringent filtering parameters AND have low error rates on shearwater
...
temp=Map(details=all.muts,pair=names(all.muts),tree=all.trees,f=function(details,pair,tree) {
  ...
  print(details[details$coding_change_chip=="Coding change mutation in driver",])
...
write_target_batches(details_comb[details_comb$bait_set=="most" & details_comb$include_in_baitset==1,],sampleID = "Comb_most_string",batch_size = 40000,coord_buffer=1,output_dir = output_dir)
write_target_batches(details_comb[details_comb$bait_set=="mod" & details_comb$include_in_baitset==1,],sampleID = "Comb_mod_string",batch_size = 40000,coord_buffer=1,output_dir = output_dir)
write_target_batches(details_comb[details_comb$bait_set=="least" & details_comb$include_in_baitset==1,],sampleID = "Comb_least_string",batch_size = 40000,coord_buffer=1,output_dir = output_dir)


#----FINAL CHECK FOR WHICH MUTATIONS ARE COVERED (after resubmitting final designs)------------------------------

cat_bed=read_in_bait_set_covered_regions(dir="/lustre/scratch119/casm/team154pc/ms56/Zur_HSCT/targeted_sequencing/baitset1/Covered_bed_files",stringency="",file_suffix="_combined.bed")

#cat_bed=read_in_bait_set_covered_regions(dir=paste0(output_dir,"Comb_covered_region_beds"),stringency="",file_suffix="_Covered.bed")
cat_bed$Chr<-gsub("chr","",cat_bed$Chr)
covered_regions_GR<-GenomicRanges::makeGRangesFromDataFrame(cat_bed,seqnames.field = "Chr",start.field = "Start",end.field = "End")
...
all.muts%>%
  ...
  write.table(file="baitset1_SNVs.bed",quote=F,sep="\t",row.names=F,col.names = F)

all.muts%>%
  dplyr::bind_rows()%>%
  filter(covered_final==1 & Mut_type=="INDEL")%>%
  dplyr::select(Chrom,Pos,Ref,Alt)%>%
  write.table(file="baitset1_INDELs.bed",quote=F,sep="\t",row.names=F,col.names = F)
